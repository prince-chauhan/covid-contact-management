...
const ContactList: React.FC = () => {
  ...
  //accessing the contacts state from the Redux store using the useSelector hook
  const contacts = useSelector((state: RootState) => state.contacts.value);

  //accessing the navigate function from the 'react-router-dom' library using the useNavigate hook to navigate through pages
  const navigate = useNavigate()

  // accessing the dispatch function from the Redux store using the useDispatch hook to perform any action on data
      ...
      {contacts.map((contact: Contact) => ( 
        <div key={contact.id} className="max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow">
            ...
            <div className='inline-flex w-full justify-evenly '>
              {/* lick event handler for the 'Edit' button that navigates to the 'edit/:id' route */}
            ...
            {/* click event handler for the 'Delete' button that dispatches the deleteContact action with the contact's ID */}
